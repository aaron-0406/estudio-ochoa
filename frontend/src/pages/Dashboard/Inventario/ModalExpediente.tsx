...
const ModalExpediente: React.FC<Props> = (props) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!!!expediente.codigo_estudio)
      return toast.warning("Codigo Estudio está vacío");

    if (!expr.digit.test(expediente.codigo_estudio))
      return toast.warning("Valores invalidos en Codigo Estudio");

    if (!!!expediente.codigo_expediente)
      return toast.warning("Codigo Expediente está vacío");

    if (!expr.digit.test(expediente.codigo_expediente))
      return toast.warning("Valores invalidos en Codigo Expediente");

    if (!!!expediente.nombre_cliente)
      return toast.warning("Nombre Cliente está vacío");

    if (!!!expediente.demanda)
      return toast.warning("Fecha de Demanda no seleccionada");

    if (!!!expediente.fecha_ep)
      return toast.warning("Fecha EP no seleccionada");

    if (!!!expediente.fecha_asignacion)
      return toast.warning("Fecha de Asignación no seleccionada");

    if (!!!expediente.id_materia)
      return toast.warning("Materia no seleccionada");

    if (!!!expediente.id_banco) return toast.warning("Banco no seleccionado");

    if (!!!expediente.estado_actual)
      return toast.warning("Estado Actual está vacío");

    if (!expr.letter.test(expediente.estado_actual))
      return toast.warning("Valores invalidos en Estado Actual");

    if (!!!expediente.contrato) return toast.warning("Contrato está vacío");

    if (!!!expediente.estado_procesal)
      return toast.warning("Estado Procesal está vacío");

    if (!!!expediente.documentos) return toast.warning("Documentos está vacío");

    if (!!!expediente.juzgado) return toast.warning("Juzgado está vacío");

    if (!!!expediente.monto) return toast.warning("Monto está vacío");

    if (!!!expediente.folio) return toast.warning("Folio está vacío");

    if (!expr.digit.test(expediente.folio))
      return toast.warning("Valores invalidos en Folio");
