...
const ModalExpediente: React.FC<Props> = (props) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!(expr.digit.test(expediente.codigo_estudio) && expr.digit.test(expediente.codigo_expediente) && expr.digit.test(expediente.folio) && expr.letter.test(expediente.estado_actual) && expediente.nombre_cliente && expediente.demanda && expediente.fecha_ep && expediente.fecha_asignacion && expediente.id_materia && expediente.id_banco && expediente.contrato && expediente.estado_procesal && expediente.documentos && expediente.juzgado && expediente.monto)) return toast.error("Campos invalidos");
    // Crear
